...
void BarterWin::Transact()
{
	...
	if(DrachFlag->GetIntValue() + DrachDifference >= 0)
	{
		...
		DrachFlag->Value = (void *) (DrachFlag->GetIntValue() + DrachDifference);
	...
	pWin->SetText(DrachFlag->GetIntValue());
}
...
//create the sub windows
	...
	pText->SetText(DrachFlag->GetIntValue());
...
BOOL BarterWin::ReceiveItem(Object *pToReceive, ZSWindow *pWinFrom, int x, int y)
{
	...
	{
		DEBUG_INFO("Doesn't know what to do with pWinFrom->GetType() in BarterWin::ReceiveItem\n");
		return FALSE;
	}
