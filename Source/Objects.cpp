...
void Object::Load(FILE *fp)
{
	...
	if ((MeshNum < -1) || (MeshNum >= Engine->GetNumMesh())) {
		DEBUG_INFO("ERROR: Object::Load fixup for MeshNum");
		MeshNum = -1;
	}
	if ((TextureNum < -1) || (TextureNum >= Engine->GetNumTextures())) {
		DEBUG_INFO("ERROR: Object::Load fixup for TextureNum");
		TextureNum = -1;
	}
