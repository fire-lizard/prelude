...
unsigned short& Combat::CreatureAreaRef(int x, int y)
{
	PTD_ASSERT(x >= rCombat.left);
	PTD_ASSERT(x < rCombat.left + COMBAT_WIDTH);
	PTD_ASSERT(y >= rCombat.top);
	PTD_ASSERT(y < rCombat.top + COMBAT_HEIGHT);
	return CreatureArea[COMBATCONVERT(x, y)];
}

unsigned short& Combat::CombatAreaRef(int x, int y)
{
	PTD_ASSERT(x >= rCombat.left);
	PTD_ASSERT(x < rCombat.left + COMBAT_WIDTH);
	PTD_ASSERT(y >= rCombat.top);
	PTD_ASSERT(y < rCombat.top + COMBAT_HEIGHT);
	return CombatArea[COMBATCONVERT(x, y)];
}
...
void Combat::FillCombatArea()
{
	...
	for(xn = rCombat.left + 1; xn < rCombat.right - 1; xn++)
	{
		...
		{
			CombatAreaRef(xn,yn) = COMBAT_LOCATION_BLOCKED;
		}
		else
		{
			CombatAreaRef(xn,yn) = COMBAT_LOCATION_EMPTY;
		}
	}
...
void inline Combat::CreautureMoveTileSurround(int xn, int yn, int AttackP, int MP)
{
	...
	if(AP < 0) return;
	if(!(CreatureAreaRef(tx, ty) & (COMBAT_LOCATION_BLOCKED | COMBAT_LOCATION_OCCUPIED)))
	{
		CreatureAreaRef(tx, ty) |= COMBAT_LOCATION_WALKABLE;
		RecordMoveLeft(tx,ty,AP - (MP + MP / 2));
		if(AP > AttackP)
			CreatureAreaRef(tx, ty) |= COMBAT_LOCATION_TIME_FOR_ATTACK;
	}
	tx++;
	if(!(CreatureAreaRef(tx, ty) & (COMBAT_LOCATION_BLOCKED | COMBAT_LOCATION_OCCUPIED)))
	{
...
/*
			...
			for(xn = StartX; xn <= EndX; xn++)
			{
				...
				{
					if(CreatureAreaRef(xn, yn) & COMBAT_LOCATION_THREATENED)
					{
						if(CreatureAreaRef(xn, yn) & COMBAT_LOCATION_VERY_THREATENED)
						{
							CreatureAreaRef(xn, yn) |= COMBAT_LOCATION_EXTREME_THREAT;
						}
						else
						{
							CreatureAreaRef(xn, yn) |= COMBAT_LOCATION_VERY_THREATENED;
						}
					}
					else
					{
						CreatureAreaRef(xn, yn) |= COMBAT_LOCATION_THREATENED;
					}
...
*/
	...
	for(Offset = 1; Offset <= MaxRange; Offset++)
	{
		...
		for(xn = ActiveX - Offset; xn < ActiveX + Offset; xn++)
		{
			...
			if(CreatureAreaRef(xn, yn) & COMBAT_LOCATION_WALKABLE)
			{
...
BOOL Combat::CheckLineOfSight(int x1, int y1, int x2, int y2, Object *pSource, Object *pTarget)
{
	...
	if(x2 == x1)
	{
		...
		{
			if(CreatureAreaRef(xn, yn) & COMBAT_LOCATION_BLOCKED)
			{
				return FALSE;
			}
		}
...
//			SafeExit("Non Creature in Combat");
	...
	while(pMod)
	{
		if(this->GetRound() >= (int)(pMod->GetStart() + pMod->GetDuration()))
		{
			this->RemoveMod(pMod);
			pTempMod = pMod;
...
int Combat::Start()
{
	...
	ZeroMemory(EnemyReferenceList,	MAX_COMBATANTS * sizeof(BOOL));
