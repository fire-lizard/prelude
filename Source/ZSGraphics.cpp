...
BOOL ZSGraphicsSystem::LoadFileIntoSurface(LPDIRECTDRAWSURFACE7 lpddInto, const char *filename, int width, int height)
{
#ifdef OPENGL
	lpddInto->bltBitmap(filename, width, height);
#else
	HBITMAP hBM;  
	BITMAP BM;  
	HDC hDCImage, hDC;  
...
LPDIRECTDRAWSURFACE7 ZSGraphicsSystem::CreateSurfaceFromFile(const char *filename, int width, int height, int mem_flags, DWORD colour_key_value)
{
	//debug_info("CreateSurfaceFromFile from %s",filename);
...
// Create the desired screen mode. 16 bpp fails to 24 when necessary.
	...
	debug_info("Windowed ? %i", Windowed);
	...
	RECT ScreenRect = {0,0,(LONG)ScreenWidth,(LONG)ScreenHeight};
...
HRESULT ZSGraphicsSystem::DrawText(RECT *rArea, const char *Text)
{
...
// patched
#ifdef OPENGL
opengl_color COLOR_SELECT = OPENGLRGB(1.0f, 1.0f, 1.0f);
opengl_color COLOR_NORMAL = OPENGLRGB(0.5f, 0.5f, 1.0f);
opengl_color COLOR_LASSO = OPENGLRGB(0.5f, 0.5f, 0.0f);
