...
int ZSButton::Draw()
{
	...
	if(Visible)
	{
		...
		if(GetText())
		{
			Engine->Graphics()->GetFontEngine()->DrawText(Engine->Graphics()->GetBBuffer(),Bounds.left, Bounds.top, GetText(), TextColor);
		}
	}	
	if(GetSibling())
	{
...
int ZSButton::LeftButtonDown(int x, int y, int doubleClick)
{
//	DEBUG_INFO("ButtonDown\n");
	...
	GetParent()->Command(ID,COMMAND_BUTTON_PRESSED,0);
	return TRUE;
}
...
//	DEBUG_INFO("ButtonUp\n");
		...
		y >= Bounds.top && y <= Bounds.bottom)
	{
		...
		GetParent()->Command(ID, COMMAND_BUTTON_CLICKED,0);
	}
	else
	{
		GetParent()->Command(ID, COMMAND_BUTTON_RELEASED,0);
	}
	return TRUE;
}
...
{
	ReleaseFocus();
	GetParent()->Command(ID, COMMAND_BUTTON_RIGHTCLICKED,0);
	return TRUE;
}
...
void ZSButton::Init()
{
	...
	//debug_info("main button surface from %s %X", BFileName,ButtonSurface);
	
	PTD_ASSERT(ButtonSurface);
...
ZSButton::ZSButton(const char *picname, int NewID, int x, int y, int width, int height, int basewidth, int baseheight, int NewNumFrames)
{
	char PicFileName[64];
