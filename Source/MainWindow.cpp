...
int ZSMainWindow::LeftButtonDown(int x, int y, int doubleClick)
{
	SetFocus(this);
		...
		y >= Bounds.top && y <= Bounds.bottom)
	{
		...
		if(pWin)
		{
			pWin->LeftButtonDown(x,y, doubleClick);
		}
		else
		{
...
//draw the mouse square
	...
	if(CurrentKeys[DIK_Q] & 0x80 && !(LastKeys[DIK_Q] & 0x80))
	{
		...
		{
			PreludeWorld->QuickSave2();
		}
	}

	if (CurrentKeys[DIK_F5] & 0x80 && !(LastKeys[DIK_F5] & 0x80)) {
		if (PreludeWorld->InCombat() || CurrentKeys[DIK_LCONTROL] & 0x80 || CurrentKeys[DIK_RCONTROL] & 0x80) {
...
void ZSMainWindow::Describe(const char *DText)
{
	if(pDescribe)
...
void ZSMainWindow::TabToNextTarget()
{
	...
	if(pTarget)
	{
		...
		long left = prMouse->left;
		long top = prMouse->top;
		this->MoveMouse(&left, &top, &LZ);
		prMouse->left = left;
		prMouse->top = top;
	}
