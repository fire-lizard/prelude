...
public:
...
#ifdef OPENGL
	opengl_primitive_strided_data stridedDataInfo;
#else
	...
	//**
#ifdef OPENGL
	HRESULT Draw(opengl_device* D3DDevice, float x, float y, float z, float angle, float scale, int frame);
#else
...
#ifdef OPENGL
	bool DrawAsEquipment(opengl_device* D3DDevice, float scale, ZSModel* target,
		float rotation, int frame, int targetFrame,
		EQUIP_POSITION equip_position, float offset,
		opengl_vector *WorldPosition,
		float targetAngle);
#else
