...
void Area::DrawTerrain()
{
	...
	if(PreludeWorld->DrawNormalChunks)
	{
		for(yn = StartY; yn <= EndY; yn++)
		{
			Offset = yn * this->ChunkWidth;
	...
	{
		//cycle though Terrains
		for(int TerrainNum = 0; TerrainNum < MAX_TERRAINS; TerrainNum++)
		{	
			Engine->Graphics()->SetTexture(Chunk::TerrainTextures[TerrainNum]);
			for(yn = StartY; yn <= EndY; yn++)
			{
				Offset = yn * this->ChunkWidth;
				for(xn = StartX; xn <= EndX; xn++)
				{
					if(BigMap[Offset + xn])
					{
						BigMap[Offset + xn]->DrawByTerrainTexture(TerrainNum);
					}
				}
			}
...
void Area::ConvertChunks()
{
	...
	for(yn = StartY; yn <= EndY; yn++)
	{
		...
		for(xn = StartX; xn <= EndX; xn++)
		{
			if(BigMap[Offset + xn])
			{
				BigMap[Offset + xn]->ConvertTerrain();
			}
		}
	}
...
void Area::Draw() {
	...
	*/
	...
	if (PreludeWorld->DrawNormalChunks) {
		...
		//Engine->Graphics()->SetRenderState(D3DRENDERSTATE_ZFUNC, D3DCMP_ALWAYS);

		//cycle though Terrains
		for (int TerrainNum = MAX_TERRAINS - 1; TerrainNum >= 0; TerrainNum--) {
			Engine->Graphics()->SetTexture(Chunk::TerrainTextures[TerrainNum]);
			for (yn = StartY; yn <= EndY; yn++) {
				Offset = yn * this->ChunkWidth;
				for (xn = StartX; xn <= EndX; xn++) {
...
{
	const D3DVECTOR* pos = pToAdd->GetPosition();

	int xUpdate = (int)pos->x;
	int yUpdate = (int)pos->y;

	PTD_ASSERT(xUpdate >= 0);
	PTD_ASSERT(xUpdate < UpdateWidth * UPDATE_SEGMENT_WIDTH);
	PTD_ASSERT(yUpdate >= 0);
	PTD_ASSERT(yUpdate < UpdateHeight * UPDATE_SEGMENT_HEIGHT);
	AddToUpdate(pToAdd, xUpdate, yUpdate);
...
void Area::AddToUpdate(Object *pToAdd, int xUpdate, int yUpdate)
{
	...
	PTD_ASSERT(SegX >= 0);
	PTD_ASSERT(SegX < UpdateWidth);
	PTD_ASSERT(SegY >= 0);
	PTD_ASSERT(SegY < UpdateHeight);
...
void Area::RemoveFromUpdate(Object *pToRemove, int xUpdate, int yUpdate)
{
	...
	if(!pToRemove->GetPrevUpdate())
	{
		D3DVECTOR *pPosition = pToRemove->GetPosition();
		...
		int SegX = xUpdate/UPDATE_SEGMENT_WIDTH;
		int SegY = yUpdate/UPDATE_SEGMENT_HEIGHT;
...
void Area::LoadNonStatic(FILE *fp)
{
	...
	fread(&ToNext,sizeof(int32_t),1,myfp);
...
{
	PTD_ASSERT(x >= 0);
	PTD_ASSERT(x < UpdateWidth* UPDATE_SEGMENT_WIDTH);
	PTD_ASSERT(y >= 0);
	PTD_ASSERT(y < UpdateHeight* UPDATE_SEGMENT_HEIGHT);

	Object *pOb = UpdateSegments[x / UPDATE_SEGMENT_WIDTH + (y/UPDATE_SEGMENT_HEIGHT)*UpdateWidth];
...
{
	return FindOtherObject(x, y, oType, NULL);
}
...
Object *Area::GetEvent(float x, float y)
{
	...
	{
		PTD_ASSERT(yn >= 0);
		PTD_ASSERT(yn < UpdateHeight* UPDATE_SEGMENT_HEIGHT);

		for(xn = PreludeWorld->UpdateRect.left; xn <= PreludeWorld->UpdateRect.right; xn++)
		{
			PTD_ASSERT(xn >= 0);
			PTD_ASSERT(xn < UpdateWidth * UPDATE_SEGMENT_WIDTH);
...
{
	Creature *pCreature = Creature::GetFirst();
...
void Area::LoadSubDynamic(RECT *rArea, const char *fname, OBJECT_T ToLoad)
{
	...
	fread(Header.Name,sizeof(char),32,fp);
	fread(&ToNext,sizeof(int32_t),1,fp);
