...
ZSFont::~ZSFont()
{
	...
	delete[] (char*)lpBits; lpBits = NULL;
...
try
{
	...
	if (!lpFontSurf[0]) {
        ...
        ReadFile(f, (LPVOID)((uintptr_t)ABCWidths+32*sizeof(ABC)),
		...
		for (c=32; c < 256; c++)
        {
			...
			BPlusC[c] = 0;
            BPlusC[c] = ABCWidths[c].abcB + ABCWidths[c].abcC;
			PTD_ASSERT(BPlusC[c] < CellWidth);
			PTD_ASSERT(BPlusC[c] >= 0);
        }
...
*/
	...
	lpFontSurf[0] = Engine->Graphics()->CreateSurfaceFromFile("fontwhite.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);	
	lpFontSurf[1] = Engine->Graphics()->CreateSurfaceFromFile("fontlightgrey.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
	lpFontSurf[2] = Engine->Graphics()->CreateSurfaceFromFile("fontdarkgrey.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
	lpFontSurf[3] = Engine->Graphics()->CreateSurfaceFromFile("fontlightgreyparch.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
	lpFontSurf[4] = Engine->Graphics()->CreateSurfaceFromFile("fontdarkgreyparch.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
	lpFontSurf[5] = Engine->Graphics()->CreateSurfaceFromFile("fontgreenparch.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
	lpFontSurf[6] = Engine->Graphics()->CreateSurfaceFromFile("fontredparch.bmp", FONT_FILE_WIDTH, FONT_FILE_HEIGHT, NULL,NULL);
...
{
	 HRESULT hr;
	PTD_ASSERT(color < NUM_FONT_COLORS);
    LPDIRECTDRAWSURFACE7 SrcSurf = Ddf->lpFontSurf[color];
	PTD_ASSERT(SrcSurf!= NULL);
...
{
	static const int MAX_LINE_COUNT = 20;
	int NumLines = 1;
	int EndPoints[MAX_LINE_COUNT];
	...
	for (n = 0; n < StringLength; n++)
	{
		...
		if(ch == '\r' || ch == '\n' || (ch == '\\' && Text[n +1] == 'n'))
		{
			if(ch == '\\' && Text[n +1] == 'n')
			{
				PTD_ASSERT(NumLines < MAX_LINE_COUNT);
	...
	for(n = 0; n < (NumLines-1); n++)
	{
		...
		int nextEndPoint = EndPoints[n + 1];
		PTD_ASSERT(nextEndPoint > endPoint);

		//allocate space for the new line
		char* pLine = new char[nextEndPoint - endPoint];
		Lines[n] = pLine;

		//copy the line from the text
		for(sn = endPoint; sn < nextEndPoint-1; sn++)
		{
			pLine[sn - endPoint] = Text[sn];
		}
		pLine[(nextEndPoint - endPoint)-1] = '\0'; 
		if((nextEndPoint > endPoint + 1) && (pLine[(nextEndPoint - endPoint) - 2] == '\\'))
		{
			pLine[(nextEndPoint - endPoint) - 2] = ' ';
		}
	}
