...
Creature::Creature(Thing *pFromThing)
{
	...
	debug_info("get texture 1 %X %X", this,pTexture);
...
//	SetData(INDEX_ID,UniqueID);
	...
	debug_info("creature scale %f", Scale);
//make it living
...
int Creature::SetData(int fieldnum, int NewValue)
{
	...
	PTD_ASSERT(fieldnum >= 0);
	PTD_ASSERT(fieldnum < NumFields);
...
int Creature::SetData(const char *fieldname, int NewValue)
{
...
int Creature::SetData(const char *fieldname, float NewfValue)
{
	//compare each data field name to the field name passed
	//when a match is found set that field to equal to value passed
	//if no match is found display an error message
...
int Creature::SetData(const char *fieldname, char *NewString)
{
	//compare each data field name to the field name passed
	//when a match is found set that field to equal to value passed
	//if no match is found display an error message
...
int Creature::SetData(const char *fieldname, D3DVECTOR *NewpVector)
{
	//compare each data field name to the field name passed
	//when a match is found set that field to equal to value passed
	//if no match is found display an error message
...
void Creature::UpdateOffScreen()
{
	...
	if(pTexture)
	{
		if(Engine->GetTextureNum(pTexture) == -1)
		{
			RECT rUpdate;
...
ACTION_RESULT_T Creature::MoveTo()
{
	...
	int EndX;
	int EndY;
	EndX = pCurAction->GetIntTarget();
	EndY = pCurAction->GetIntData();
	...
	if(!pCurAction->IsFinished())
	{
		...
		else
		{
			if(Large && (!Valley->IsClear(CheckX, CheckY, this)))
			{
				Object *pOb;
				pOb = Valley->FindObject(CheckX,CheckY, OBJECT_PORTAL);
				if(!pOb && Valley->IsClear(CheckX+1, CheckY, this))
					pOb = Valley->FindObject(CheckX+1,CheckY, OBJECT_PORTAL);
				if(!pOb && Valley->IsClear(CheckX, CheckY+1, this))
					pOb = Valley->FindObject(CheckX,CheckY+1, OBJECT_PORTAL);
				if(!pOb && Valley->IsClear(CheckX+1, CheckY+1, this))
...
ACTION_RESULT_T Creature::MoveIn() 
{
	...
	if(!pCurAction->IsStarted())
	{
		...
		float DestAngle;
		CurAngle = GetData(INDEX_ROTATION).fValue;
		DestAngle = DIRECTIONANGLES[pCurAction->GetIntData()];
...
*/
	...
	SetData(INDEX_ROTATION,DIRECTIONANGLES[pCurAction->GetIntData()]);
...
ACTION_RESULT_T Creature::Rotate() 
{
	...
	if(!pCurAction->IsStarted())
	{
		...
		EndRotation = DIRECTIONANGLES[pCurAction->GetIntData()];
...
ACTION_RESULT_T Creature::Animate() 
{
	if(!pCurAction->IsStarted())
	{
		...
		ANIMATION_RANGE_T RangeNum = (ANIMATION_RANGE_T)pCurAction->GetIntData();
		int Start;
		int End;
...
{ 
	...
	EncodedNum = pCurAction->GetIntData();
	CastAura *pAura = NULL;
...
ACTION_RESULT_T Creature::Approach() 
{
	...
	NeededDistance = (float)pCurAction->GetIntData() + 0.5f;
...
ACTION_RESULT_T Creature::WaitUntil() 
{
	...
	{
		pCurAction->Start();
		pCurAction->SetLength(pCurAction->GetIntTarget());
		pCurAction->SetFrame(0);
		return ACTION_RESULT_ADVANCE_FRAME;
	}
...
ACTION_RESULT_T Creature::Open()
{
	...
	switch(pThing->GetObjectType())
	{
		case OBJECT_PORTAL:
			...
			int opentype;
			opentype = pCurAction->GetIntData();
			switch(opentype)
			{
...
{
#ifdef BACKPORT_18
	if (!Engine->Graphics()->GetUnknown_var_18()) return 1;
#endif
...
int LoadBinCreatures(FILE *fp)
{
	...
	for(n = 0; n < NewNumCreatures -1; n++)
	{
		...
		for(int en = 0; en < MAX_EQUIPMENT; en++)
		{
			...
			if(bEquipAt)
			{
				Object *pObject = LoadObject(fp);
				PTD_ASSERT(pObject != NULL);
				pCreature->Equipment[en] = (GameItem *)pObject;
...
int Creature::Equip(GameItem *ToEquip, int NumEquip)
{
	...
	// From Ghidra
	{
		...
		local_8 = 0xffffffff;
		puStack12 = &LAB_00548f3e;
		local_10 = *in_FS_OFFSET;
		*in_FS_OFFSET = &local_10;
		puVar4 = local_1fc;
		for (iVar9 = 0x7b; iVar9 != 0; iVar9 = iVar9 + -1) {
			*puVar4 = 0xcccccccc;
			puVar4 = puVar4 + 1;
		}
		local_24 = 0;
		local_14 = this;
		BVar1 = Party::IsMember(&PreludeParty, this);
		if (BVar1 != 0) {
			piVar2 = (int*)GameItem::GetData(local_10c);
			(*(code*)((local_14->parent).vtbl)->GetData1)(local_110, 0x31);
			piVar3 = (int*)__chkesp();
			if (*piVar3 < *piVar2) {
				(*(code*)((local_14->parent).vtbl)->GetData1)(local_114, 0);
				puVar4 = (undefined4*)__chkesp();
				sprintf(local_a4, "%s is not strong enough to equip that.", (char)*puVar4);
				Describe(local_a4);
				goto LAB_0043b664;
			}
			piVar2 = (int*)GameItem::GetData(local_118);
			(*(code*)((local_14->parent).vtbl)->GetData1)(local_11c, 0x32);
			piVar3 = (int*)__chkesp();
			if (*piVar3 < *piVar2) {
				(*(code*)((local_14->parent).vtbl)->GetData1)(local_120, 0);
				puVar4 = (undefined4*)__chkesp();
				sprintf(local_a4, "%s is not coordinated enough to equip that.", (char)*puVar4);
				Describe(local_a4);
				goto LAB_0043b664;
			}
			BVar1 = World::InCombat(PreludeWorld);
			if (BVar1 != 0) {
				(*(code*)((local_14->parent).vtbl)->GetData1)(local_124, 0x12);
				piVar2 = (int*)__chkesp();
				if (*piVar2 < 4) {
					(*(code*)((local_14->parent).vtbl)->GetData1)(local_12c, 0);
					puVar4 = (undefined4*)__chkesp();
					sprintf(local_a4, "%s needs 3 AP to equip during combat.", (char)*puVar4);
					Describe(local_a4);
					goto LAB_0043b664;
				}
				ResponseB = "No";
				ResponseA = "Yes";
				pcVar10 = "Spend 3 AP to equip?";
				pZVar5 = ZSWindow::GetMain();
				BVar1 = Confirm(pZVar5, pcVar10, ResponseA, ResponseB);
				if (BVar1 == 0) goto LAB_0043b664;
				(*(code*)((local_14->parent).vtbl)->GetData1)(local_128, 0x12);
				piVar2 = (int*)__chkesp();
				(*(code*)((local_14->parent).vtbl)->SetData4)(0x12, *piVar2 + -3);
				__chkesp();
			}
		}
		ppcVar6 = (char**)GameItem::GetData(local_130);
		local_1c = *ppcVar6;
		iVar9 = _strcmp(local_1c, "HAND");
		if (iVar9 == 0) {
			piVar2 = (int*)GameItem::GetData(local_134);
			if ((*piVar2 == 2) &&
				((local_20 = (GameItem*)Creature::GetEquipment((Creature*)"LEFTHAND", (char*)unaff_EDI),
					local_20 != (GameItem*)0x0 ||
					(local_20 = (GameItem*)Creature::GetEquipment((Creature*)"RIGHTHAND", (char*)unaff_EDI),
						local_20 != (GameItem*)0x0)))) goto LAB_0043b664;
			local_18 = Thing::GetIndex(local_14, "RIGHTHAND");
			(*(code*)((local_14->parent).vtbl)->GetData1)(local_138, local_18);
			piVar2 = (int*)__chkesp();
			if ((*piVar2 != 0) &&
				((piVar2 = (int*)GameItem::GetData(local_13c), *piVar2 < 2 &&
					(piVar2 = (int*)GameItem::GetData(ToEquip, (int)local_140), *piVar2 == 2)))) {
				piVar2 = (int*)GameItem::GetData(local_144);
				local_ac = *piVar2;
				if ((local_ac != 0) && (local_ac < 100)) {
					puVar4 = (undefined4*)GameItem::GetData(local_148);
					local_1b0 = *puVar4;
					switch (local_1b0) {
					case 0:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_14c, 0x39);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 1:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_150, 0x3a);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 2:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_154, 0x3b);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 3:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_158, 0x3c);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 4:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_15c, 0x3d);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 5:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_160, 0x3e);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					default:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_164, 0x3f);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
						break;
					case 7:
						(*(code*)((local_14->parent).vtbl)->GetData1)(local_168, 0x40);
						piVar2 = (int*)__chkesp();
						local_a8 = *piVar2;
					}
					if (local_a8 < local_ac) {
						BVar1 = Party::IsMember(&PreludeParty, local_14);
						if (BVar1 != 0) {
							sprintf(local_ec, "Requires %i skill to wield in off hand.", (char)local_ac);
							Describe(local_ec);
						}
					}
					else {
						local_18 = Thing::GetIndex(local_14, "LEFTHAND");
						local_24 = 1;
					}
				}
			}
			...
			iVar9 = _strcmp(local_1c, "RING");
			if (iVar9 == 0) {
				local_18 = Thing::GetIndex(local_14, "RIGHTRING");
				(*(code*)((local_14->parent).vtbl)->GetData1)(local_16c, local_18);
				piVar2 = (int*)__chkesp();
				if (*piVar2 != 0) {
					local_18 = Thing::GetIndex(local_14, "LEFTRING");
				}
			}
			else {
				local_18 = Thing::GetIndex(local_14, local_1c);
			}
		}
		iVar9 = _strcmp(local_1c, "LEFTHAND");
		if (((iVar9 != 0) ||
			(local_20 = (GameItem*)Creature::GetEquipment((Creature*)"RIGHTHAND", (char*)unaff_EDI),
				local_20 == (GameItem*)0x0)) ||
			(piVar2 = (int*)GameItem::GetData(local_174 + 4), *piVar2 != 2)) {
			iVar9 = Thing::GetIndex(local_14, "HEAD");
			local_18 = local_18 - iVar9;
			local_20 = (GameItem*)FUN_00431ac0(local_18);
			if (local_20 == (GameItem*)0x0) {
				local_180 = (GameItem*)operator_new(0x68);
				local_8 = 0;
				if (local_180 == (GameItem*)0x0) {
					local_1b8 = (GameItem*)0x0;
				}
				else {
					local_1b8 = ToEquip;
					GameItem::GameItem(local_180, ToEquip);
				}
				local_17c = local_1b8;
				local_8 = 0xffffffff;
				*(GameItem**)((int)local_14->Equipment + local_18 * 4 + 3) = local_1b8;
				if (NumEquip != 0) {
					GameItem::SetQuantity(*(GameItem**)((int)local_14->Equipment + local_18 * 4 + 3), NumEquip);
				}
				GameItem::SetLocation
				(*(GameItem**)((int)local_14->Equipment + local_18 * 4 + 3), 2, &local_14->parent);
				iVar9 = Thing::GetIndex(local_14, "HEAD");
				local_18 = local_18 + iVar9;
				piVar2 = (int*)GameItem::GetData(ToEquip, (int)local_184);
				(*(code*)((local_14->parent).vtbl)->SetData4)(local_18, *piVar2 + NumEquip * 1000);
				__chkesp();
				Creature::CreateTexture(unaff_EDI);
				Creature::SetEquipObjects(unaff_EDI);
				piVar2 = (int*)GameItem::GetData(ToEquip, (int)local_188);
				local_1bc = *piVar2;
				if ((local_1bc == 2) && (iVar9 = _strcmp(local_1c, "RIGHTHAND"), iVar9 == 0)) {
					(*(code*)((local_14->parent).vtbl)->GetData1)(local_18c, 0x18);
					piVar2 = (int*)__chkesp();
					piVar3 = (int*)GameItem::GetData(local_190);
					(*(code*)((local_14->parent).vtbl)->SetData4)(0x18, *piVar2 + -5 + *piVar3);
					__chkesp();
					(*(code*)((local_14->parent).vtbl)->SetData3)(0x1e, 0x3fc00000);
					__chkesp();
				}
				piVar2 = (int*)GameItem::GetData(local_194);
				local_f0 = *piVar2;
				if (local_f0 != 0) {
					Push(local_14);
					EventManager::RunEvent((EventManager*)&PreludeEvents, local_f0);
				}
				pcVar10 = "MOD1";
				pIVar8 = GameItem::GetItem(ToEquip, (Item*)&DAT_005500a4);
				local_f4 = Thing::GetIndex(pIVar8, pcVar10);
				for (local_fc = 0; local_fc < 5; local_fc = local_fc + 1) {
					piVar2 = (int*)GameItem::GetData(ToEquip, (int)local_198);
					local_f8 = *piVar2;
					if (local_f8 != 0) {
						local_100 = fopen("debug.txt", &DAT_0054e29c);
						fprintf(local_100, "Modding due to equipment being equipped.\n");
						fclose(local_100);
						puVar4 = (undefined4*)GameItem::GetData(ToEquip, (int)local_19c);
						(*(code*)((local_14->parent).vtbl)->GetData0)(local_1a0, *puVar4);
						piVar2 = (int*)__chkesp();
						if (0 < *piVar2) {
							puVar4 = (undefined4*)GameItem::GetData(ToEquip, (int)local_1a4);
							(*(code*)((local_14->parent).vtbl)->GetData0)(local_1a8, *puVar4);
							piVar2 = (int*)__chkesp();
							iVar9 = *piVar2 + local_f8;
							puVar4 = (undefined4*)GameItem::GetData(ToEquip, (int)local_1ac);
							(*(code*)((local_14->parent).vtbl)->SetData0)(*puVar4, iVar9);
							__chkesp();
						}
					}
					local_f4 = local_f4 + 2;
				...
				if (BVar1 != 0) {
					iVar9 = 0x1046a;
					pZVar5 = ZSWindow::GetMain();
					local_104 = ZSWindow::GetChild(pZVar5, iVar9);
					if (local_104 != (ZSWindow*)0x0) {
						local_108 = local_104;
						CharacterWin::ResetStats((CharacterWin*)local_104);
					}
				}
				BVar1 = World::InCombat(PreludeWorld);
				if (BVar1 != 0) {
					Creature::DoneOneAction(local_14);
				}
			}
			else {
				iVar9 = _strcmp(local_1c, "AMMO");
				if (iVar9 == 0) {
					uVar11 = 1;
					pIVar8 = (Item*)local_174;
					local_1b4 = GameItem::GetItem(local_20, pIVar8);
					(*(code*)((local_1b4->parent).vtbl)->GetData1)(pIVar8, uVar11);
					piVar2 = (int*)__chkesp();
					piVar3 = (int*)GameItem::GetData(ToEquip, (int)local_178);
					if (*piVar2 == *piVar3) {
						iVar9 = GameItem::GetQuantity(local_20);
						iVar7 = GameItem::GetQuantity(ToEquip);
						GameItem::SetQuantity(local_20, iVar9 + iVar7);
					}
				}
			}
		}
	...
	if(PreludeParty.IsMember(this))
	{
		...
		{
			sprintf(blarg, "%s is not strong enough to equip that.",this->GetData(INDEX_NAME).String);
			Describe(blarg);
			return FALSE;
		}
		...
		{
			sprintf(blarg, "%s is not coordinated enough to equip that.",this->GetData(INDEX_NAME).String);
			Describe(blarg);
			return FALSE;
		}
		...
		if(PreludeWorld->InCombat())
		{
			...
			{
				sprintf(blarg, "%s needs 3 AP to equip during combat.",this->GetData(INDEX_NAME).String);
				Describe(blarg);
				return FALSE;
			}
		}
...
ACTION_RESULT_T Creature::GameCommand()
{
	...
	Command = (ACTION_GAME_COMMAND_T)pCurAction->GetIntTarget();
	switch(Command)
	{
...
int Creature::Give(Thing *ToReceive, int Quantity)
{
	if(PreludeParty.IsMember(this) && ToReceive->GetData(INDEX_ID).Value == 100)
	{
		...
		pFlag->Value = (void *) (pFlag->GetIntValue() + Quantity);
		char blarg[64];
...
void Creature::operator = (Creature &OtherThing)
{
	for(int n = 0; n < NumFields; n++)
	{
		switch(DataTypes[n])
			...
			for(int n = 0; n < pCreature->GetNumLocators(); n++)
			{
				...
				if((pLocator->GetStart() < pLocator->GetEnd() &&
					...
					(pLocator->GetStart() > pLocator->GetEnd() &&
					 (pLocator->GetStart() <= Hour || pLocator->GetEnd() >= Hour)))
				{
					Area* pLocatorArea = PreludeWorld->GetArea(pLocator->GetArea());
					...
					if((rLoc.right - rLoc.left) >= 1 || (rLoc.bottom - rLoc.top) >= 1)
					{
						...
						{
							int testX = (rand() % (rLoc.right - rLoc.left)) + rLoc.left;
							int testY = (rand() % (rLoc.bottom - rLoc.top)) + rLoc.top;
							if(pLocatorArea->IsClear(testX, testY))
							{
								NewX = testX;
								NewY = testY;
								break;
					...
					PTD_ASSERT(NewX >= 0);
					PTD_ASSERT(NewX < pLocatorArea->GetWidth());
					PTD_ASSERT(NewY >= 0);
					PTD_ASSERT(NewY < pLocatorArea->GetHeight());
					pPosition = pCreature->GetPosition();
					pPosition->x = (float)NewX + 0.5f;
					pPosition->y = (float)NewY + 0.5f;
					pPosition->z = pLocatorArea->GetTileHeight(pPosition->x,pPosition->y);
					pLocatorArea->AddToUpdate(pCreature);
