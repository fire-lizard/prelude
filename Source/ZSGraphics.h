...
public:
#ifdef BACKPORT_18
	int GetUnknown_var_18() {
		return unknown_var_18;
	}

	void SetUnknown_var_18(int val) {
		unknown_var_18 = val;
	}
#endif
	...
	LPDIRECTDRAWSURFACE7 CreateSurfaceFromFile(const char *filename, int width, int height, int mem_flags, DWORD colour_key_value);
	BOOL LoadFileIntoSurface(LPDIRECTDRAWSURFACE7 lpddInto, const char *filename, int width, int height);
	...
	HRESULT DrawText(RECT *rArea, const char *Text);
