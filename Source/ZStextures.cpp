...
void ZSTexture::Load(const char * fname, LPDIRECT3DDEVICE7 D3DDevice, LPDIRECTDRAW7 DirectDraw, DWORD *Mask, int w = 0, int h = 0) {
	...
	debug_info(fname);
	hr = D3DXCreateTextureFromFile(D3DDevice, NULL,(LPDWORD)&ww,(LPDWORD)&hh,&sf,NULL,&Surface,&NumMaps,(LPSTR)fname, D3DX_FT_LINEAR);

//	D3DXCheckTextureRequirements(D3DDevice,&format,(LPDWORD)&ww,(LPDWORD)&hh,&sf);

// 	hr = D3DXCreateTextureFromFile(D3DDevice,&format,(LPDWORD)&ww,(LPDWORD)&hh,&sf,NULL,&Surface,&NumMaps,(LPSTR)fname, D3DX_FT_POINT);
...
// 	hr = D3DXCreateTextureFromFile(D3DDevice,NULL,(LPDWORD)&ww,(LPDWORD)&hh,&sf,NULL,&Surface, &NumMaps, fname, D3DX_FT_POINT);
	...
	if(!(*Mask))
	{
		...
		RGBMask = (DWORD)(Desc.ddpfPixelFormat.dwRBitMask | Desc.ddpfPixelFormat.dwGBitMask | Desc.ddpfPixelFormat.dwBBitMask );
 		...
 		PTD_ASSERT(Desc.lpSurface != NULL);
		SurfPtr = (DWORD* ) (( BYTE* )Desc.lpSurface);
		
		*Mask = *SurfPtr;
		*Mask &= RGBMask;
		debug_info("set keymask %X %X", *Mask, RGBMask);
 		Surface->Unlock( NULL );
	}
	else
	{
...
*/
	...
	debug_info("OverLayName mask %s %i", OverLayName,fp);
	if(fp)
	{
		fclose(fp);
...
ZSTexture::ZSTexture(const char * filename, LPDIRECT3DDEVICE7 D3DDevice, LPDIRECTDRAW7 DirectDraw, DWORD *Mask, int w = 0, int h = 0) 
{	
	Used = FALSE;
	pPrev = NULL;
	pNext = NULL;
...
ZSTexture::ZSTexture(LPDIRECT3DDEVICE7 D3DDevice, int NewWidth, int NewHeight, DWORD *Mask)
{
	Used = FALSE;
	pPrev = NULL;
	pNext = NULL;
...
//	D3DXCheckTextureRequirements(D3DDevice,&format,(LPDWORD)&ww,(LPDWORD)&hh,&sf);
	...
	PTD_ASSERT(hr == S_OK);
//	PTD_ASSERT(sf == D3DX_SF_A1R5G5B5);
//	PTD_ASSERT(Width == ww);
//	PTD_ASSERT(Height == hh);
