...
void ZSEdit::AddChar(int ASCIICode)
{
	...
	for(n = CharOffset; n < (int)strlen(OldText); n++)
	{
	...
	if(GetParent())
	{
		GetParent()->Command(ID, COMMAND_EDIT_CHANGED,ASCIICode);
	}
...
void ZSEdit::BackSpace()
{
	...
	if(strlen(OldText))
	{
		...
		if(GetParent())
		{
			GetParent()->Command(ID, COMMAND_EDIT_CHANGED,0);
		}
		CharOffset--;
	}
	else
...
void ZSEdit::Delete()
{
	...
	if(OldText && CharOffset)
	{
		if(strlen(OldText))
		{
			if(CharOffset >= (int)strlen(OldText))
			{
				return;
			}
	...
	for(n = CharOffset+1; n < (int)strlen(OldText); n++)
	{
		NewText[n-1] = OldText[n];
	}
...
int ZSEdit::LeftButtonDown(int x, int y, int doubleClick)
{
	if(pInputFocus != this)
	{
		SetFocus(this);
...
//this is going to be big
	...
	if((Pressed(DIK_RETURN) && !LastPressed(DIK_RETURN)) || (Pressed(DIK_NUMPADENTER) && !LastPressed(DIK_NUMPADENTER)))
	{
		...
		if(GetParent())
		{
			GetParent()->Command(ID, COMMAND_EDIT_DONE, 0);
		}
		return TRUE;
	}
	...
	if(Pressed(DIK_LCONTROL) || Pressed(DIK_RCONTROL))
	{
		...
		{
			strcpy(ClipBoard,GetText());	
			return TRUE;
		}
		if(Pressed(DIK_V) && !LastPressed(DIK_V))
		{
			SetText(ClipBoard);
			SetOffset(strlen(GetText()));
			return TRUE;
		}
	}
	...
	{
		if(GetText() && CharOffset < (int)strlen(GetText()))
		{
			CharOffset++;
			return TRUE;
		}
...
int ZSEdit::Draw()
{
	...
	if(Visible)
	{
		...
		if(GetText())
		{
			Engine->Graphics()->GetFontEngine()->DrawText(Engine->Graphics()->GetBBuffer(), Bounds.left + Border, Bounds.top + Border, GetText(), TextColor);
		}
		if(DrawCursor)
		{
			RECT CursorRect;
			CursorRect.left = Border + Bounds.left + Engine->Graphics()->GetFontEngine()->GetTextWidth(GetText(),0,CharOffset);
...
void ZSEdit::GainFocus()
{
	...
	if(GetParent())
	{
		GetParent()->Command(ID,COMMAND_CHILD_GAINFOCUS,NULL);
	}
	return;
}
...
void ZSEdit::LoseFocus()
{
	...
	if(GetParent())
	{
		GetParent()->Command(ID,COMMAND_CHILD_LOSEFOCUS,NULL);
	}
	return;
}
