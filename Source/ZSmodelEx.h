...
#ifdef OPENGL
	opengl_vector GetPointVector(int Point, int Frame, float Angle);

	opengl_vertex GetPoint(int Point, int Frame);

	HRESULT Draw(opengl_device* D3DDevice, float x, float y, float z, float angle, int frame);
	HRESULT Draw(opengl_device* D3DDevice, float x, float y, float z, float angle, float xscale, float yscale, float zscale, int frame);
	HRESULT Draw(opengl_device* D3DDevice, int frame);

	HRESULT DrawLit(opengl_device* D3DDevice, float x, float y, float z, float angle, float xscale, float yscale, float zscale, int frame);
#else
...
#ifdef OPENGL
	BOOL Intersect(int Frame, opengl_vector *vRayStart, opengl_vector *vRayEnd);
	BOOL LineIntersect(int Frame, opengl_vector *vLineStart, opengl_vector *vLineEnd);
#else
