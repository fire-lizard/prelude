...
int ZSText::Draw()
{
	if(Visible)
	{
		...
		Engine->Graphics()->GetFontEngine()->DrawText(Engine->Graphics()->GetBBuffer(), &DrawTo, GetText(), TextColor);
	}
	if(GetSibling())
	{
		GetSibling()->Draw();
...
ZSText::ZSText(int NewID, int x, int y, const char *string)
{
	int Width;
...
ZSText::ZSText(int NewID, int x, int y, const char *string, int NewDuration)
{
	int Width;
...
ZSText::ZSText(int NewID, int x, int y, int width, int height, const char *string, int NewDuration, BOOL Parchment)
{
	ID = NewID;
