...
int RegionEditWin::LeftButtonDown(int x, int y, int doubleClick)
{
	ZSWindow *pWin;
	EditPoint *pPoint;
...
int FloorToolWin::Command(int IDFrom, int Command, int Param)
{
	...
	{
		return GetParent()->Command(IDFrom, Command, Param);
	}
	return TRUE;
}
...
int RoofToolWin::Command(int IDFrom, int Command, int Param)
{
	...
	{
		GetParent()->Command(IDFrom,Command,Param);
	}
	return TRUE;
}
