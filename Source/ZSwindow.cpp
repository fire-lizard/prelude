...
int ZSWindow::SetText(const char *NewText) 
{ 
	if(NewText == NULL) {
		if (Text) {
			delete[] Text;
	...
	if(newTextLen + 1 > TextSize)
	{
		if (Text) {
			delete[] Text;
			Text = NULL;
		}
		TextSize = newTextLen + 1;
...
//LeftButtonDown
//
int ZSWindow::LeftButtonDown(int x, int y, int doubleClick)
{ 
	ZSWindow *pWin;
...
int ZSWindow::DoubleClick(int x, int y) {
	return 0;
}
...
LPDIRECTDRAWSURFACE7 ZSWindow::CreatePortrait(const char *PortraitName, BOOL GetFace)
{
...
#ifdef OPENGL
	BackGroundSurface = Engine->Graphics()->CreateSurface(Bounds.right - Bounds.left, Bounds.bottom - Bounds.top, NULL, D3DRGB(1.0f,0,1.0f).dword());
	Engine->Graphics()->FillSurface(BackGroundSurface, D3DRGB(1.0f, 0, 1.0f).dword(), NULL);
#else
	...
	{	
		fclose(fp);
#ifdef OPENGL
		lpddPor = Engine->Graphics()->CreateSurfaceFromFile(FileName,Dim,Dim,NULL,D3DRGB(1.0f,0,1.0f).dword());
#else
