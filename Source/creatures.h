...
//	int RemoveItem(Thing *pRemoveThing);
	...
	int SetData(const char *fieldname, int NewValue);	
	int SetData(const char *fieldname, float NewfValue);
	int SetData(const char *fieldname, char *NewString);
	int SetData(const char *fieldname, D3DVECTOR *NewpVector);
	...
	BOOL KnowsSpell(int Num) { PTD_ASSERT(Num >= 0); PTD_ASSERT(Num < MAX_SPELLS); return KnownSpells[Num]; }
...
// Debug ----------------------------------------------
	...
	void operator = (Creature &OtherThing);
