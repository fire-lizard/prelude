...
//	ZeroMemory(&RegionHeader,sizeof(RegionHeader));
	...
	Engine->Input()->initCameraDevice(&vCamera, &vLookAt, &vWorldUp);
...
//autosave if neccessary
		if(AutosaveRate)
		{
			if((int)((CurTime - LastAutosave) / 60000) >= AutosaveRate &&
...
void World::SetCamera(float xAngle) {
	CameraAngle = xAngle;
...
void World::SaveGame(const char *filename, const char *GameID)
{
	...
	if (sizeof(size_t) == 8) {
		...
		//x86
		sprintf(ver, "%s--x86--linux", VERSION_NUMBER);
	}
#else
#ifdef _WIN64
	sprintf(ver, "%s--x64--windows", VERSION_NUMBER);
#else
	sprintf(ver, "%s--x86--windows", VERSION_NUMBER);
#endif
#endif
	fwrite(ver, 1, 32, fp);
...
void World::LoadGame(const char *filename)
{
	...
	fread(&OldPos,sizeof(uint32_t),1,fp);
	PTD_ASSERT(Pos == OldPos);
