...
//------------------------------------------------------------------------------	
		...
		ZSWindow *GetParent() {	return pParent;	}
		ZSWindow* GetChild() { if (pChild) PTD_ASSERT(pChild->pParent == this); return pChild; }
		ZSWindow* GetSibling() { if (pSibling) PTD_ASSERT(pSibling->pParent == pParent); return pSibling; }
...
//------------------------------------------------------------------------
		...
		int SetText(const char *NewText);
		int SetText(int n);
		...
		virtual int LeftButtonDown(int x, int y, int doubleClick);
