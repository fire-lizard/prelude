...
CreatePartyWin::CreatePartyWin()
{
	...
	if(fp)
	{
		...
		BackGroundSurface = Engine->Graphics()->CreateSurfaceFromFile("StartScreen.bmp",800,600,0,0);
	}
		...
		if(IDFrom == IDC_SELECTBACKGROUND)
		{
			...
			if(pWin && pWin->IsVisible())
			{
				...
				if(BNum != -1)
				{
					...
					SA = Pop();
					MaxCombat = CurCombat = SA->GetIntValue();
					delete SA;

					SA = Pop();
					MaxNon = CurNon = SA->GetIntValue();
					delete SA;

					SA = Pop();
					MaxFree = CurFree = SA->GetIntValue();
					delete SA;
...
CreateCharacterWin::CreateCharacterWin(Creature *pToCreate)
{
	...
	BackGroundSurface = Engine->Graphics()->CreateSurfaceFromFile("charcreation.bmp",800,600,0,0);
...
void CreateCharacterWin::AddSkill(int Num)
{
	...
	if(Num - StartSkill <= NumCombat)
	{
		if(CurCombat)
		{
			...
			ScriptArg *SA;
			SA = Pop();
			pTarget->SetData(Num, SA->GetIntValue());
		}
		else
		{
			if(CurFree)
